...
const StudentDetail: React.FC<StudentDetailProps> = ({ id }) => {
  ...
  const isEditing = !!fichaParaEditarOuCriar;
  ...
  const { data: studentWorkouts = [], isLoading: isLoadingWorkouts, error: errorLoadingWorkouts, refetch: refetchFichasAluno } = useQuery<RotinaListagemItem[], Error>({
    queryKey: ["fichasAluno", studentId], // Manter "fichasAluno" por enquanto para não quebrar a query key, renomearemos depois se necessário
  ...
  const handleViewFichaClick = (rotina: RotinaListagemItem) => {
    ...
    const fichaViewData: FichaTreinoView = { // Este tipo será renomeado para RotinaTreinoView
        _id: rotina._id,
    ...
    if (Array.isArray(studentWorkouts) && studentWorkouts.length > 0) {
      return (
        <div className="space-y-4">
          {studentWorkouts.map((rotina) => ( // Renomeado 'ficha' para 'rotina'
            <div key={rotina._id} className="border rounded-lg p-4 shadow-sm dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div className="flex-shrink-0 flex gap-2 self-start sm:self-center">
                ...
                <Button variant="destructive" size="icon" className="h-8 w-8" title="Excluir Rotina" onClick={() => handleDeleteClick(rotina)} disabled={deleteMutation.isPending && deleteMutation.variables === rotina._id} >
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            Rotinas de Treino Atribuídas {/* Renomeado */}
            <Button size="sm" onClick={handleCreateFichaClick} disabled={!studentId || !student}>
              <Dumbbell className="w-4 h-4 mr-2" /> Nova Rotina para {student?.nome?.split(' ')[0]} {/* Renomeado */}
            </Button>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {renderFichasContent()} {/* A função interna ainda usa 'ficha', mas o título acima foi mudado */}
        </CardContent>
      </Card>
      ...
      {isTreinoFormModalOpen && (
          <TreinoFormModal
              ...
              fichaParaEditar={fichaParaEditarOuCriar} // A prop em TreinoFormModal ainda é fichaParaEditar
              alunoId={!isEditing ? studentId : undefined}
          />
      )}
      {isViewFichaModalOpen && fichaParaVisualizar && (
          <FichaViewModal // Este componente será renomeado depois
              isOpen={isViewFichaModalOpen}
              onClose={() => setIsViewFichaModalOpen(false)}
              ficha={fichaParaVisualizar} // A prop em FichaViewModal ainda é ficha
